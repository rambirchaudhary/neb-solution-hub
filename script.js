/* ==========================================================================
   ...
   - Vanilla JS (ES6)
   - Accessible interactions, theme persistence, mobile menu, scroll reveal,
     counters, carousel, mindmap toggles, FAQ accordion, lazy loading, toasts,
     keyboard shortcuts, ripple, and small UI helpers.
   ========================================================================== */
...
function processToastQueue() {
  if (!toastQueue.length) { toastTimer = null; toastEl.hidden = true; return; }
  ...
  toastTimer = setTimeout(() => { toastEl.hidden = true; toastTimer = null; setTimeout(processToastQueue, 200); }, duration);
}
...
function initMobileMenu() {
  ...
  function openMenu() { mobileMenu.hidden = false; mobileToggle.setAttribute('aria-expanded', 'true'); mobileMenu.querySelector('a, button, input')?.focus(); document.body.style.overflow = 'hidden'; }
  function closeMenu() { mobileMenu.hidden = true; mobileToggle.setAttribute('aria-expanded', 'false'); mobileToggle.focus(); document.body.style.overflow = ''; }
  mobileToggle.addEventListener('click', (e) => { e.stopPropagation(); if (mobileMenu.hidden) openMenu(); else closeMenu(); });
  mobileLinks.forEach(l => l.addEventListener('click', () => closeMenu()));
  document.addEventListener('click', (e) => { if (!mobileMenu.hidden && !mobileMenu.contains(e.target) && !mobileToggle.contains(e.target)) closeMenu(); });
...
function initTypingEffect() {
  ...
  function tick() {
    ...
    if (forward) {
      ...
      if (char === current.length) { forward = false; setTimeout(tick, 1200); return; }
    } else {
      el.textContent = current.slice(0, --char);
...
function initScrollReveal() {
  ...
  if (!revealEls.length || !('IntersectionObserver' in window)) { revealEls.forEach(el => { el.style.opacity = 1; el.style.transform = 'none'; }); return; }
  ...
  revealEls.forEach(el => { el.style.opacity = 0; el.style.transform = 'translateY(12px)'; observer.observe(el); });
}
...
function initCounters() {
  ...
  if (!('IntersectionObserver' in window)) { statsEls.forEach(s => animateCounter(s)); return; }
...
function initFAQ() {
  faqButtons.forEach(btn => {
    ...
    btn.addEventListener('keydown', (e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); toggleFAQ(btn); } });
  });
}
...
function initMindmapToggles() {
  branchNodes.forEach(node => {
    ...
    node.addEventListener('keydown', (e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); toggleBranch(node); } });
  });
  subNodes.forEach(node => {
    node.setAttribute('role', 'button');
    node.setAttribute('tabindex', '0');
    node.setAttribute('aria-expanded', 'false');
    node.addEventListener('click', (e) => { e.stopPropagation(); toggleSub(node); });
    node.addEventListener('keydown', (e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); e.stopPropagation(); toggleSub(node); } });
  });
}
...
function initNewsletter() {
  ...
  newsletterForm.addEventListener('submit', (e) => {
    ...
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) { showToast('Please enter a valid email address', true); newsletterEmail?.focus(); return; }
...
function initBackToTop() {
  if (!backToTop) return;
  window.addEventListener('scroll', debounce(() => { if (window.scrollY > 600) backToTop.hidden = false; else backToTop.hidden = true; }, 80));
...
function initActiveNavOnScroll() {
  ...
  const obs = new IntersectionObserver(entries => {
    entries.forEach(entry => {
      ...
      if (entry.isIntersecting) { navLinks.forEach(n => n.classList.remove('active')); if (link) link.classList.add('active'); }
    });
  }, { threshold: 0.45 });
...
/* -------------------------
   Smooth internal link scroll + focus
   ------------------------- */
document.addEventListener('click', (e) => {
  ...
  if (target) {
    ...
    setTimeout(() => { target.setAttribute('tabindex', '-1'); target.focus({ preventScroll: true }); }, 450);
  }
});
